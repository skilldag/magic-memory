...
async function main() {
  ...
  ok(r1.nodeRepulsion >= 12000 && r1.nodeRepulsion <= 80000, 'nodeRepulsion in range')
  ok(r1.gravity >= 0.02 && r1.gravity <= 0.12, 'gravity in range')
  ok(r1.padding >= 20 && r1.padding <= 80, 'padding in range')
  ok(r1.numIter >= 200 && r1.numIter <= 1200, 'numIter in range')
  ...
  ok(r3.nodeRepulsion >= 12000, 'large graph: repulsion scales up')
  ok(r3.numIter <= 1200, 'large graph: numIter max clamped')

  // Test 4: single node — 800x600, 1 node
  const r4 = calcAdaptiveLayoutParams(800, 600, 1)
  ok(r4.numIter >= 200, 'single node: min iter')
