 ...
 * - 层级分组和统计
 */
export function analyzeGraph(concepts: Concept[], edges: ConceptEdge[]): GraphSummary {
 ...
 * 将图谱摘要格式化为可读的文本报告
 */
export function formatSummaryToString(summary: GraphSummary): string {
  const lines: string[] = []
  ...
  lines.push(`| 指标 | 值 |`)
  lines.push(`|------|-----|`)
  lines.push(`| 总概念数 | ${summary.stats.totalConcepts} |`)
  lines.push(`| 总边数 | ${summary.stats.totalEdges} |`)
  lines.push(`| 根节点 | ${summary.stats.rootsCount} |`)
  lines.push(`| 叶子节点 | ${summary.stats.leavesCount} |`)
  lines.push(`| 平均度 | ${summary.stats.averageDegree} |`)
  lines.push(`| 密度 | ${summary.stats.density} |`)
  lines.push(`| 跨层跳跃 | ${summary.stats.crossLayerJumpsCount} |`)
  lines.push('')
  lines.push(`**边类型分布：** ${JSON.stringify(summary.stats.byEdgeType)}`)
  lines.push(`**层级分布：** ${JSON.stringify(summary.stats.byLevel)}`)
  lines.push(`**分类分布：** ${JSON.stringify(summary.stats.byCategory)}`)
  lines.push('')

  // 根节点
  lines.push('## 根节点（入度为 0）')
  lines.push('')
  if (summary.rootConcepts.length === 0) {
    lines.push('无根节点')
  } else {
    for (const r of summary.rootConcepts) {
  ...
  lines.push('')
  if (summary.leafConcepts.length === 0) {
    lines.push('无叶子节点')
  } else {
    for (const l of summary.leafConcepts) {
  ...
  lines.push('')
  if (summary.longestPaths.length === 0) {
    lines.push('未发现数据流路径')
  } else {
    for (const p of summary.longestPaths) {
  ...
  lines.push('')
  if (summary.hubConcepts.length === 0) {
    lines.push('无枢纽节点')
  } else {
    lines.push('| 排名 | 概念 | Level | 入度 | 出度 | 总计 | depends_on | leads_to | related |')
    lines.push('|------|------|-------|------|------|------|------------|----------|---------|')
    summary.hubConcepts.slice(0, 15).forEach((h, i) => {
  ...
  // 跨层跳跃
  if (summary.crossLayerJumps.length > 0) {
    lines.push('## 跨层跳跃')
    lines.push('')
    for (const j of summary.crossLayerJumps.slice(0, 10)) {
  ...
  lines.push('## 层级分组')
  lines.push('')
  for (const g of summary.levelGroups) {
